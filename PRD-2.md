...
Student arrives → Staff encodes in e-logbook → Registrar ticks reviewed and signed →
...
System auto-calculates process duration → Student returns → Staff records claimer + signature →
...
| UC-5 | View all incomplete/unreleased requests | Admin & Staff |
| UC-6 | Generate summary reports (daily, periodic) | Admin |
| UC-7 | Manage staff accounts | Admin |
| UC-8 | Add a new student to the database | Admin & Staff|
